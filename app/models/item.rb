class Item < ApplicationRecord
  ...
  def best_day
    invoices.joins(:invoice_items, :transactions).where(transactions: {result: 0}).select("invoices.created_at, (COUNT(invoice_items.item_id = #{id})  AS count)").group(:created_at).order(count: :desc).limit(1)
  end
end
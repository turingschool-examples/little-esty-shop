...
describe 'merchants items index' do
  ...
  describe 'display' do
    ...
    it 'next to most popular items I see the date with the most sales for each item' do
      ...
      visit merchant_items_path(merchant_6)
      within '#top_five_items' do
        expect(page).to have_content("Top Selling Date: #{invoice_6.created_at}")
        ...
        expect(page).to have_content("Top Selling Date: #{invoice_10.created_at}")
        expect(page).to have_content("Top Selling Date: #{invoice_9.created_at}")
        expect(invoice_6.created_at).to appear_before(invoice_7.created_at)
        expect(invoice_7.created_at).to appear_before(invoice_8.created_at)
        expect(invoice_8.created_at).to appear_before(invoice_10.created_at)
        expect(invoice_10.created_at).to appear_before(invoice_9.created_at)
      end
    end
  end
end

...
# require 'webmock'
# require 'httparty'
RSpec.describe 'the merchant discount new page' do
  ...
  it 'has link to create a new discount' do
    ...
    fill_in(:min_quantity, with: 25)
    fill_in(:percent_off, with:35)
    click_button 'Create discount'
    expect(current_path).to eq("/merchants/#{merchant_1.id}/discounts")
    expect(page).to have_content("Discount percent off: 35")
    expect(page).to have_content("Discount minimum quantity: 25")

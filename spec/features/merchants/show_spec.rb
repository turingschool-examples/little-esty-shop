...
RSpec.describe 'merchant show' do
  ...
  describe 'Items Ready to Ship Section' do
    ...
    it 'orders list from oldest invoice creation to newest' do
      ...
      customer1 = Customer.create!(first_name: 'Kyle', last_name: 'Ledin')
      invoice1 = Invoice.create!(status: 1, customer_id: customer1.id, created_at: Time.now)
      invoice2 = Invoice.create!(status: 1, customer_id: customer1.id, created_at: Time.now - 5.days)
      invoice3 = Invoice.create!(status: 1, customer_id: customer1.id, created_at: Time.now - 6.days)
      InvoiceItem.create!(quantity: 5, unit_price: item1.unit_price, item_id: item1.id,
                                invoice_id: invoice1.id)
      InvoiceItem.create!(quantity: 5, unit_price: item2.unit_price, item_id: item2.id,
                                invoice_id: invoice2.id)
      InvoiceItem.create!(quantity: 5, unit_price: item3.unit_price, item_id: item3.id,
                                invoice_id: invoice3.id)

...
RSpec.describe 'merchant invoices page', type: :feature do
  before :each do
    ...
    InvoiceItem.create!(item_id: @item1.id.to_s, invoice_id: @inv1.id.to_s)
    InvoiceItem.create!(item_id: @item4.id.to_s, invoice_id: @inv2.id.to_s)
  end

  it 'can see all the invoices(and id) that have at least one of my merchants items', :vcr do
    # As a merchant,
  ...
  it 'has a link on each id to the merchant invoice show page', :vcr do

...
RSpec.describe "merchant dashboard", type: :feature do
  ...
  it 'lists top 5 customers and number of successful transactions for each customer' do
    ...
    invoice_2 = customer_2.invoices.create!(status: 1, created_at: Time.now, updated_at: Time.now)
    invoice_3 = customer_3.invoices.create!(status: 1, created_at: Time.now, updated_at: Time.now)
    invoice_4 = customer_4.invoices.create!(status: 1, created_at: Time.now, updated_at: Time.now)
    invoice_5 = customer_5.invoices.create!(status: 1, created_at: Time.now, updated_at: Time.now)
    invoice_6 = customer_6.invoices.create!(status: 1, created_at: Time.now, updated_at: Time.now)
    invoice_7 = customer_2.invoices.create!(status: 1, created_at: Time.now, updated_at: Time.now)
    ...
    invoice_11 = customer_6.invoices.create!(status: 1, created_at: Time.now, updated_at: Time.now)
    invoice_12 = customer_6.invoices.create!(status: 1, created_at: Time.now, updated_at: Time.now)
    invoice_13 = customer_6.invoices.create!(status: 1, created_at: Time.now, updated_at: Time.now)
    invoice_14 = customer_7.invoices.create!(status: 1, created_at: Time.now, updated_at: Time.now)
    ...
    expect("1.James Franco").to appear_before("2.Frank Jameson")
    expect("2.Frank Jameson ").to appear_before("3.John Smith")
    expect("3.John Smith").to appear_before("4.Zack Adams")
    expect("4.Zack Adams ").to appear_before("5.Chloe Wheeler")
    within("#customer-0") do
      expect(page).to have_content("1.James Franco - 4")
      expect(page).to_not have_content("Frank Jameson - 3")
      expect(page).to_not have_content("John Smith - 3")
      expect(page).to_not have_content("Zack Adams - 2")
      expect(page).to_not have_content("Chloe Wheeler - 2")
    end
    within("#customer-1") do
      expect(page).to have_content("2. Frank Jameson - 3")
      expect(page).to_not have_content("James Franco - 4")
      expect(page).to_not have_content("John Smith - 3")
      expect(page).to_not have_content("Chloe Wheeler - 1")
      expect(page).to_not have_content("Zack Adams - 2")
    end
    within("#customer-2") do
      expect(page).to have_content("3. John Smith - 3")
      expect(page).to_not have_content("James Franco - 4")
      expect(page).to_not have_content("Frank Jameson - 3")
      expect(page).to_not have_content("Zack Adams - 2")
      expect(page).to_not have_content("Chloe Wheeler - 1")
    ...
    within("#customer-3") do
      expect(page).to have_content("4. Zack Adams - 2")
      expect(page).to_not have_content("Chloe Wheeler - 1")
      expect(page).to_not have_content("James Franco - 4")
      expect(page).to_not have_content("Frank Jameson - 3")
      expect(page).to_not have_content("John Smith - 3")
    ...
    within("#customer-4") do
      expect(page).to have_content("5. Chloe Wheeler - 1")
      expect(page).to_not have_content("James Franco - 4")
      expect(page).to_not have_content("Frank Jameson - 3")
      expect(page).to_not have_content("John Smith - 3")
      expect(page).to_not have_content("Zack Adams - 2")

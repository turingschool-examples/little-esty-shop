...
RSpec.describe "Merchant's dashboard", type: :feature do
  describe "as merchant" do
    before(:each) do
      ...
      @transaction_7 = @invoice_5.transactions.create(credit_card_number: 1234123412341234, credit_card_expiration_date: '2012-03-27', result: 'success')
      ...
      @transaction_10 = @invoice_6.transactions.create(credit_card_number: 1234123412341234, credit_card_expiration_date: '2012-03-27', result: 'failed')
    ...
    it "I see the 5 customer names with the most successful transactions" do
      ...
      expect(@customer_5.name).to appear_before(@customer_1).name
    ...
    it 'I also see each customers number of succesful transactions' do
      expect(page).to have_content "#{@customer_5.name} - 3 purchases"
      expect(page).to have_content "#{@customer_1.name} - 2 purchases"
      expect(page).to have_content "#{@customer_2.name} - 2 purchases"
      expect(page).to have_content "#{@customer_3.name} - 1 purchases"
      expect(page).to have_content "#{@customer_4.name} - 1 purchases"
    end
  end
end

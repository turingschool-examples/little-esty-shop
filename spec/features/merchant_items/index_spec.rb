...
RSpec.describe 'Merchant Items Index Page' do
  ...
  describe "Merchant Items Index: Top Item's Best Day" do
    ...
    let!(:invoice_item1) { create(:invoice_item, item: items[0], invoice: invoice1, quantity: 42, unit_price: 500000000) }
    let!(:invoice_item7) { create(:invoice_item, item: items[0], invoice: invoice1, quantity: 6, unit_price: 1) }
    let!(:invoice_item2) { create(:invoice_item, item: items[1], invoice: invoice2, quantity: 3, unit_price: 2000) }
    let!(:invoice_item8) { create(:invoice_item, item: items[1], invoice: invoice3, quantity: 22, unit_price: 2) }
    let!(:invoice_item3) { create(:invoice_item, item: items[2], invoice: invoice2, quantity: 5, unit_price: 3000) }
    let!(:invoice_item9) { create(:invoice_item, item: items[2], invoice: invoice3, quantity: 63, unit_price: 3) }
    let!(:invoice_item4) { create(:invoice_item, item: items[3], invoice: invoice3, quantity: 16, unit_price: 4000) }
    let!(:invoice_item10) { create(:invoice_item, item: items[3], invoice: invoice3, quantity: 1, unit_price: 4) }
    let!(:invoice_item5) { create(:invoice_item, item: items[4], invoice: invoice2, quantity: 2, unit_price: 6000) }
    let!(:invoice_item11) { create(:invoice_item, item: items[4], invoice: invoice2, quantity: 7, unit_price: 5) }
    ...
    it "has the date with the most sales for each item" do
      ...
      within "#topItem-#{items[2].id}" do
        expect(page).to have_content("Top selling date for #{items[2].name} was #{invoice3.created_at.strftime("%m/%d/%y")}")
      end

      within "#topItem-#{items[3].id}" do
        expect(page).to have_content("Top selling date for #{items[3].name} was #{invoice3.created_at.strftime("%m/%d/%y")}")
      end

      within "#topItem-#{items[4].id}" do
        expect(page).to have_content("Top selling date for #{items[4].name} was #{invoice2.created_at.strftime("%m/%d/%y")}")
      end

      within "#topItem-#{items[5].id}" do
        expect(page).to have_content("Top selling date for #{items[5].name} was #{invoice3.created_at.strftime("%m/%d/%y")}")
      end
    end
  end
end

...
RSpec.describe 'the merchant invoice show page' do
  ...
  # let!(:invoice_item_4) {InvoiceItem.create!(invoice_id: invoice_1.id, item_id: item_4.id, quantity: 30, unit_price: 1, status: 'pending', created_at: Time.new(2021))}
  ...
  it 'displays discounted total revenue for merchants invoices' do
    ...
    expect(page).to have_content('Total Discounted Revenue: 46')
    save_and_open_page
  end

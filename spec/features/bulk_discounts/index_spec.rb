...
RSpec.describe 'Merchant Bulk Discount Index', type: :feature do
  ...
  it 'Will display all bulk discounts, including links to individual show pages' do
    ...
    within '#bulk-discounts' do
      expect(page).to have_link("View Bulk Discounts", to: merchant_bulk_discounts_index_path(@merchant.id))
    end

    click_link("View Bulk Discounts")

    expect(current_path).to eq(merchant_bulk_discounts_index_path(@merchant.id))
    ...
    within "#discounts" do
      within "#discount-#{@bulk_discount1.id}" do
        ...
        expect(page).to have_link("View Details", to: merchant_bulk_discounts_show_path(@bulk_discount1.id))
      end

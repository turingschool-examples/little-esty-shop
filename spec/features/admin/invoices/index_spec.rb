...
RSpec.describe "Admin Invoices Index", type: :feature do
  ...
  it "Links from index to show page for each invoice", :vcr do
    ...
    within("#invoices") do
      within("#invoice-#{@invoices.first.id}") do
        expect(page).to have_link(@invoices.first.id, href: admin_invoice_path("#{@invoices.first.id}"))
        expect(page).to_not have_link(@invoices.second.id, href: admin_invoice_path("#{@invoices.second.id}"))
        expect(page).to_not have_link(@invoices.last.id, href: admin_invoice_path("#{@invoices.last.id}"))
      end
    end

    click_link(@invoices.first.id)

    expect(current_path).to eq(admin_invoice_path("#{@invoices.first.id}"))
  end
end

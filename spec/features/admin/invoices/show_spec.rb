...
RSpec.describe 'Admin Invoice Show' do
  before :each do
    ...
    @ii1 = InvoiceItem.create!(item_id: @item1.id.to_s, invoice_id: @inv1.id.to_s, quantity: 100, unit_price: 1000,
                               status: 1)
    @ii2 = InvoiceItem.create!(item_id: @item2.id.to_s, invoice_id: @inv1.id.to_s, quantity: 200, unit_price: 2000,
                               status: 1)
    @ii3 = InvoiceItem.create!(item_id: @item4.id.to_s, invoice_id: @inv2.id.to_s, quantity: 300)
  end

  it 'displays invoice information including id, status, customer and created at', :vcr do
    visit("/admin/invoices/#{@inv1.id}")

    expect(page).to have_content(@inv1.id.to_s)
    expect(page).to have_content(@inv1.status.to_s)
    expect(page).to have_content(@inv1.created_at.strftime('%A, %B%e, %Y'))
    expect(page).to have_content('Mark Ruffalo')
  end

  it 'displays all items and their attributes', :vcr do
    visit("/admin/invoices/#{@inv1.id}")
  ...
  it 'displays total revenue that will be generated by invoice', :vcr do
    visit("/admin/invoices/#{@inv1.id}")
    expect(page).to have_content('Total Revenue: 5000.00')
  end

  it 'displays invoice status as a select field and gives ability to update status', :vcr do
    visit("/admin/invoices/#{@inv1.id}")

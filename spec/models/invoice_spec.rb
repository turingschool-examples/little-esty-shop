...
RSpec.describe Invoice, type: :model do
  ...
  let!(:invoice_1) {customer_1.invoices.create!(status: 'in progress' )}
  let!(:invoice_2) {customer_1.invoices.create!(status: 'in progress' )}
  let!(:invoice_3) {customer_1.invoices.create!(status: 'in progress' )}
  ...
  it 'discounted total revenue' do
    expect(invoice_1.discounted_total_revenue).to eq(51)
  end

  it "returns all invoice_items for invoices that are not complete" do
    expect(Invoice.incomplete_invoices).to eq([invoice_item_2, invoice_item_3, invoice_item_4])
  end

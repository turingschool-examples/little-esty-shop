...
RSpec.describe Invoice, type: :model do
  ...
  before :each do
      ...
      @seven = BulkDiscount.create!(name: 'Seven', percent_discount: 0.07, quantity_threshold: 7, merchant_id: @merchant3.id)
      @two = BulkDiscount.create!(name: 'Two', percent_discount: 0.02, quantity_threshold: 2, merchant_id: @merchant3.id)
      ...
      @invoice_item13 = InvoiceItem.create!(item_id: @item2.id, invoice_id: @invoice7.id, quantity: 5, unit_price: 2175, status: 2)
      @invoice_item7 = InvoiceItem.create!(item_id: @item6.id, invoice_id: @invoice7.id, quantity: 2, unit_price: 2175, status: 2)
      @invoice_item14 = InvoiceItem.create!(item_id: @item8.id, invoice_id: @invoice7.id, quantity: 57, unit_price: 934, status: 2)
      @invoice_item23 = InvoiceItem.create!(item_id: @item10.id, invoice_id: @invoice7.id, quantity: 3, unit_price: 3450, status: 2)
      @invoice_item21 = InvoiceItem.create!(item_id: @item13.id, invoice_id: @invoice7.id, quantity: 23, unit_price: 2345, status: 2)
      @invoice_item24 = InvoiceItem.create!(item_id: @item14.id, invoice_id: @invoice7.id, quantity: 6, unit_price: 2175, status: 2)
      @invoice_item22 = InvoiceItem.create!(item_id: @item16.id, invoice_id: @invoice7.id, quantity: 11, unit_price: 934, status: 2)


      @invoice_item1 = InvoiceItem.create!(item_id: @item1.id, invoice_id: @invoice1.id, quantity: 6, unit_price: 2400, status: 1)
      @invoice_item2 = InvoiceItem.create!(item_id: @item2.id, invoice_id: @invoice2.id, quantity: 12, unit_price: 3450, status: 0)
      ...
      @invoice_item6 = InvoiceItem.create!(item_id: @item4.id, invoice_id: @invoice6.id, quantity: 7, unit_price: 76000, status: 2)

      @invoice_item8 = InvoiceItem.create!(item_id: @item8.id, invoice_id: @invoice1.id, quantity:25, unit_price: 934, status: 2)
      @invoice_item9 = InvoiceItem.create!(item_id: @item2.id, invoice_id: @invoice2.id, quantity: 21, unit_price: 3450, status: 2)
      @invoice_item10 = InvoiceItem.create!(item_id: @item4.id, invoice_id: @invoice3.id, quantity: 1, unit_price: 76000, status: 2)
      @invoice_item11 = InvoiceItem.create!(item_id: @item1.id, invoice_id: @invoice4.id, quantity: 6, unit_price: 2400, status: 2)
      @invoice_item12 = InvoiceItem.create!(item_id: @item5.id, invoice_id: @invoice5.id, quantity: 11, unit_price: 2345, status: 2)
      ...
      @invoice_item17 = InvoiceItem.create!(item_id: @item6.id, invoice_id: @invoice2.id, quantity: 8, unit_price: 2175, status: 2)
      ...
      @invoice_item20 = InvoiceItem.create!(item_id: @item9.id, invoice_id: @invoice11.id, quantity: 11, unit_price: 2375, status: 2)
      ...
      @invoice_item27 = InvoiceItem.create!(item_id: @item11.id, invoice_id: @invoice9.id, quantity:34, unit_price: 2375, status: 2)
      @invoice_item28 = InvoiceItem.create!(item_id: @item15.id, invoice_id: @invoice10.id, quantity: 2, unit_price: 5405, status: 2)
      @invoice_item29 = InvoiceItem.create!(item_id: @item16.id, invoice_id: @invoice10.id, quantity:53, unit_price: 934, status: 2)
      ...
      @invoice_item32 = InvoiceItem.create!(item_id: @item12.id, invoice_id: @invoice12.id, quantity: 22, unit_price: 1908, status: 2)
      @invoice_item33 = InvoiceItem.create!(item_id: @item11.id, invoice_id: @invoice13.id, quantity: 2, unit_price: 2375, status: 2)
      @invoice_item34 = InvoiceItem.create!(item_id: @item10.id, invoice_id: @invoice14.id, quantity: 17, unit_price: 3450, status: 2)
      ...
      @invoice_item38 = InvoiceItem.create!(item_id: @item16.id, invoice_id: @invoice9.id, quantity: 57, unit_price: 934, status: 2)
  ...
  it 'can calculate a discount' do

   expect(@invoice7.apply_discount).to eq(true)
  end

  xit 'return bulk discounts' do

    expect(@merchant1.discounts).to eq(true)
   # expect(@invoice11.sucessful_transactions.count).to eq(0)
  end
  xit 'will determine if an invoice has sucessful transactions' do

    expect(@invoice7.sucessful_transactions.count).to eq(1)
   # expect(@invoice11.sucessful_transactions.count).to eq(0)
  end

  xit 'will list the merchant discounts' do

    expect(@invoice7.merchant_discounts).to eq([@five, @ten, @fifteen, @fifty, @six, @eleven, @sixteen, @fifty_one])
   # expect(@invoice11.sucessful_transactions.count).to eq(0)
  end
end

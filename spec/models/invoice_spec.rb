...
RSpec.describe Invoice, type: :model do
  ...
  describe 'class methods' do
    ...
    let!(:invoice_item3) { InvoiceItem.create!(item_id: item_2.id, invoice_id: invoice_3.id, quantity: 4, unit_price: 1000, status: 2)}
    ...
    let!(:invoice_item6) { InvoiceItem.create!(item_id: item_3.id, invoice_id: invoice_5.id, quantity: 3, unit_price: 2500, status: 1)}
    ...
    it '#successful_transactions' do
      expect(Invoice.successful_transactions).to eq(0)
    end

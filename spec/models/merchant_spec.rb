...
describe 'Merchant model methods' do
  ...
  before :each do
    ...
    @item1 = @merchant1.items.create!(name: "cheese", description: "european cheese", unit_price: 2400, item_status: 1)
    @item2 = @merchant1.items.create!(name: "onion", description: "red onion", unit_price: 3450, item_status: 1)
    @item3 = @merchant1.items.create!(name: "earing", description: "Lotus earings", unit_price: 14500)
    @item4 = @merchant1.items.create!(name: "bracelet", description: "Silver bracelet", unit_price: 76000, item_status: 1)
    @item5 = @merchant1.items.create!(name: "ring", description: "lotus ring", unit_price: 2345)
  end

  describe 'enable and disable item status from index page' do
    it 'will sort item by status' do
      expect(@merchant1.enabled_status).to eq([@item1, @item2, @item4])
      expect(@merchant1.disabled_status).to eq([@item3, @item5])
    end
  end

...
RSpec.describe Merchant do
  ...
  describe 'instance methods' do
    ...
    let!(:invoice1) { customer1.invoices.create!(status: 2, created_at: "2012-03-27 14:54:09 UTC") }
    let!(:invoice2) { customer2.invoices.create!(status: 2, created_at: "2012-03-27 14:54:09 UTC") }
    let!(:invoice3) { customer3.invoices.create!(status: 1, created_at: "2012-03-27 14:54:09 UTC") }
    let!(:invoice4) { customer4.invoices.create!(status: 2, created_at: "2012-03-27 14:54:09 UTC") }
    let!(:invoice5) { customer5.invoices.create!(status: 2, created_at: "2012-03-27 14:54:09 UTC") }
    let!(:invoice6) { customer6.invoices.create!(status: 2, created_at: "2012-03-27 14:54:09 UTC") }

    let!(:invoice_item1) { InvoiceItem.create!(item_id: item1.id, invoice_id: invoice1.id, quantity: 5, unit_price: 130, status: "shipped") }
    let!(:invoice_item2) { InvoiceItem.create!(item_id: item1.id, invoice_id: invoice2.id, quantity: 10, unit_price: 130, status: "shipped") }
    let!(:invoice_item3) { InvoiceItem.create!(item_id: item2.id, invoice_id: invoice3.id, quantity: 8, unit_price: 220, status: "packaged") }
    let!(:invoice_item4) { InvoiceItem.create!(item_id: item3.id, invoice_id: invoice4.id, quantity: 1, unit_price: 100, status: "shipped") }
    let!(:invoice_item5) { InvoiceItem.create!(item_id: item4.id, invoice_id: invoice5.id, quantity: 2, unit_price: 15, status: "shipped") }
    let!(:invoice_item6) { InvoiceItem.create!(item_id: item5.id, invoice_id: invoice6.id, quantity: 3, unit_price: 12, status: "shipped") }
    let!(:invoice_item7) { InvoiceItem.create!(item_id: item4.id, invoice_id: invoice6.id, quantity: 1, unit_price: 16, status: "shipped") }
    let!(:invoice_item8) { InvoiceItem.create!(item_id: item5.id, invoice_id: invoice5.id, quantity: 2, unit_price: 12, status: "shipped") }
    let!(:invoice_item9) { InvoiceItem.create!(item_id: item6.id, invoice_id: invoice1.id, quantity: 4, unit_price: 35, status: "shipped") }
    let!(:invoice_item10) { InvoiceItem.create!(item_id: item7.id, invoice_id: invoice4.id, quantity: 1, unit_price: 35, status: "shipped") }

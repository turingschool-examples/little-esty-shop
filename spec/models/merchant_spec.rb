  ...
  describe 'instance methods' do
    ...
    describe '::top_merchants' do
      it 'returns the top five customers by revenue' do
        ...
        item = merchant1.items.create(name: 'Mug', description: 'You can drink with it', unit_price: 5)
        item = merchant2.items.create(name: 'GPU', description: 'Its too expensive', unit_price: 1500)
        item = merchant3.items.create(name: 'Compressed Air', description: 'Its compressed', unit_price: 2)
        ...
        expect(Customer.top_merchants).to eq([merchant1, merchant2])
      end
    end
  end
end

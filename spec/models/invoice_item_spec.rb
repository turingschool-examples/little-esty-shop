...
RSpec.describe InvoiceItem, type: :model do
  describe "Class Methods" do
    describe "::incomplete_item_invoices" do
      it "it returns all invoice items where status is 'pending' or 'packaged' sorted by oldest to newest" do
        customer_1 = create(:customer)

        invoice_1 = create(:invoice, customer_id: customer_1.id, created_at: Date.new.next)
        ...
        expect(InvoiceItem.incomplete_item_invoices).to eq([invoice_item_2, invoice_item_1])
      end
    end
  end
end

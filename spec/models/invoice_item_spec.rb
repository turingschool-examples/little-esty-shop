...
RSpec.describe InvoiceItem, type: :model do
  before :each do
    ...
    @invoice_item3 = InvoiceItem.create!(item_id: @item3.id, invoice_id: @invoice3.id, quantity: 1, unit_price: 14_500,
                                         status: 2)
    @invoice_item4 = InvoiceItem.create!(item_id: @item7.id, invoice_id: @invoice4.id, quantity: 17, unit_price: 5405,
                                         status: 2)
  ...
  describe 'instance methods' do
    ...
    it 'calculate_discounted_renevue' do
      expect(@invoice_item4.calculate_discounted_renevue).to eq(77_183.4)
      expect(@invoice_item1.calculate_discounted_renevue).to eq(13_536.0)
    end

    it 'calculate_non_discounted_renevue' do
      expect(@invoice_item4.calculate_renevue).to eq(91_885)
      expect(@invoice_item1.calculate_renevue).to eq(14_400)
    end
  end
end
